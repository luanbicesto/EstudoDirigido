...
public class LocalSearchWrapper {
	...
	private void applyNSwap(CCPChromosome chromosome, int n) {
		...
		// Apply swap
		applyNSwapMove(chromosome, nodes, bestSteps, bestType, bestConfigurationContribution, currentContribution,
				clusters);
	...
	private double computeNSwapContribution(int steps, int type, CCPChromosome chromosome, ArrayList<Integer> nodes,
	...
	private boolean validNSwapStep(int steps, int type, CCPChromosome chromosome, ArrayList<Integer> nodes,
	...
	private void migrateNode(int node, int targetCluster, CCPChromosome chromosome, double fitnessDifference)
			throws Exception {
	...
	private boolean canMigrateToCluster(int node, int targetCluster, CCPChromosome chromosome, boolean nodeInCluster) {
		...
		try {
			...
			targetClusterFutureWeight = chromosome.getCurrentClustersWeight().get(targetCluster)
					+ instance.getNodeWeights()[node];
	...
	public void swapNodes(int node1, int node2, double swapImprovement, CCPChromosome chromosome) throws Exception {
		...
		futureClustersWeight[0] = chromosome.getCurrentClustersWeight().get(clusterOfNodes[0]) - nodeWeight[0]
				+ nodeWeight[1];
		futureClustersWeight[1] = chromosome.getCurrentClustersWeight().get(clusterOfNodes[1]) + nodeWeight[0]
				- nodeWeight[1];
	...
	private void swapNodesByCluster(Integer node1, Integer node2, int clusterNode1, int clusterNode2,
			CCPChromosome chromosome) {
	...
	public double computeSwapImprovement(int node1, int node2, CCPChromosome chromosome) {
		...
		try {
			...
			if (canSwapNodes(node1, node2, chromosome, clusterOfNodes)) {
				...
				swapImprovement = ((originalContribution[0] + originalContribution[1]) * -1) + newContribution[0]
						+ newContribution[1];
			}
